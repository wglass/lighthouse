...
class Writer(ConfigWatcher):
    """
    The coordinator configuration writing class.

    This ConfigWatcher subclass manages Discovery and Cluster and Coordinator
    ...
    change takes place the Coordinator instances are notified and syncs their
    config file contents with the updated clusters.
    """

    watched_configurables = (Coordinator, Discovery, Cluster)
    ...
    def wait_for_updates(self):
        ...
        configured coordinators sync their config files.
        """
        while True:
            ...
            for coordinator in self.configurables[Coordinator].values():
                coordinator.sync_file(self.configurables[Cluster].values())
            self.nodes_updated.clear()

    def on_coordinator_add(self, coordinator):
        """
        Once a coordinator is added we set the `nodes_updated` event so that we
        sync the config file right away.
        """
        coordinator.sync_file(self.configurables[Cluster].values())
        self.nodes_updated.set()

    def on_coordinator_update(self, name, new_config):
        """
        Sets the `nodes_updated` event so that we sync coordinator config files
        whenever a coordinator is updated.
        """
        self.nodes_updated.set()

    def on_coordinator_remove(self, name):
        """
        The removal of a coordinator config is unusual but still supported.

        If the coordinator being removed is the only configured one we fire
        a critical log message saying so.  A writer setup with no coordinators
        is less than useless.
        """
        if len(self.configurables[Coordinator]) == 1:
            logger.critical(
                "'%s' config file removed! no more coordinators left!", name
            )
    ...
    def on_cluster_update(self, name, new_config):
        ...
        `nodes_updated` flag is set so that we properly sync any coordinator
        configurations.
        """
